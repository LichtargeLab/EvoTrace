...
```{r}
ComputeSCWzscore(basS_10A_bg, resi_mut)
```

```{r}
ComputeSCWzscore(basS_10A_AF70_bg, resi_mut)
```

As negative controls, the random selected positions have non significant z scores.
```{r}
ComputeSCWzscore(basS_10A_bg, resi_random)
```

```{r}
ComputeSCWzscore(basS_10A_AF70_bg, resi_random)
```
