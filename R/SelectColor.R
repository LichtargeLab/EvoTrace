...
SelectColor <- function(color_type = c("ET", "red_white", "red_white_blue", "white_red",
                        ...
                        prefix = "#") {
  if (color_type == "ET") {
    # auto generate the colors, use same implementation as pyETv.
    # The values might be slightly different from pyETv after converting to RGB in pymol.
    # pyETv is passing RGB values to pymol not hex code.
    colorRange <- hsv((0:99)/100*0.8,1,1)
    ...
    colorRange <- c(rep("#90d1e2", 30),
                    rep("#ffba57", 40),
                    rep("#ff0000", 30))
  } else if (color_type == "rev_EA_bin") {
    colorRange <- c(rep("#90d1e2", 30),
                    rep("#ffba57", 40),
                    rep("#ff0000", 30))
    colorRange <- rev(colorRange)
  } else if (color_type == "red_yellow_green") {
    # auto generate the colors, use same implementation as pyETv.
    colorRange <- hsv((0:99)/100*0.33,1,1)
  }
  if (prefix == "0x") {
